...
// CALCULATIONS
function calcResidentialElev(numFloors, numApts) {
    console.log("GGetting residential calcs");
    let url = "http://localhost:3004/calc/residential?floors="+numFloors+"&apts="+numApts;
    //GET
    fetch(url)
    .then((response) => response.json())
    .then((data) => {
        displayCalcElv_input.value = data.elevators_required;
    });
}
function calcCommercialElev(numFloors, maxOccupancy) {
    console.log("GGetting commercial calcs");
    let url = "http://localhost:3004/calc/commercial?floors="+numFloors+"&occupancy="+maxOccupancy;
    //GET
    fetch(url)
    .then((response) => response.json())
    .then((data) => {
...
function displayPricing(productLine, numElv, buildingType) {
    let url = "http://localhost:3004/calc/"+buildingType+"?elevators="+numElv+"&tier="+productLine;
    let subtotal;
    let totalInstallFee;
    let totalPrice;
    let unitPrice;
...
function updatePricingDisplay() {
    if (!displayCalcElv_input.value) {
        ...
        try {
            ...
            displayPricing(productLine, numElv, buildingType);
        } catch {
            // empty: waiting for user to select product line;
        }
    }
